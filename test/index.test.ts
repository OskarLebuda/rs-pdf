...
describe('pdfInfo', () => {
  it('returns basic metadata', async () => {
    const info = await pdfInfo(SAMPLE_PDF);
  ...
  it('throws on missing file', async () => {
    await expect(pdfInfo('/tmp/does-not-exist.pdf')).rejects.toThrow('Failed to open PDF');
  });
});
...
describe('pdfToHtml', () => {
  it('converts all pages to HTML with SVG visual layer and HTML text layer', async () => {
    const result = await pdfToHtml(SAMPLE_PDF, { seoTextLayer: true });
  ...
  it('omits text layer by default (seoTextLayer unset)', async () => {
    const result = await pdfToHtml(SAMPLE_PDF, { startPage: 0, endPage: 0 });
    expect(result.html).toContain('<svg');
  ...
  it('respects page range (multiple pages)', async () => {
    const result = await pdfToHtml(SAMPLE_PDF, { startPage: 0, endPage: 1 });
  ...
  it('throws on invalid page range', async () => {
    await expect(pdfToHtml(SAMPLE_PDF, { startPage: 10, endPage: 5 })).rejects.toThrow('Invalid page range');
  });

  it('throws on missing file', async () => {
    await expect(pdfToHtml('/tmp/does-not-exist.pdf')).rejects.toThrow('Failed to open PDF');
  });

  it('throws on DRM-protected PDF without password', async () => {
    const result = await pdfToHtml(SAMPLE_PDF);
    expect(result.isDrmProtected).toBe(false);
  });
  ...
  it('respects dpi option', async () => {
    const low = await pdfToHtml(SAMPLE_PDF, { startPage: 0, endPage: 0, dpi: 72 });
    const high = await pdfToHtml(SAMPLE_PDF, { startPage: 0, endPage: 0, dpi: 300 });
...
describe('pdfPageToHtml', () => {
  it('returns a page fragment with correct metadata', async () => {
    const page = await pdfPageToHtml(SAMPLE_PDF, 0, { seoTextLayer: true });
  ...
  it('fragment is not a full document', async () => {
    const page = await pdfPageToHtml(SAMPLE_PDF, 0);
  ...
  it('throws on out-of-range pageIndex', async () => {
    const info = await pdfInfo(SAMPLE_PDF);
    await expect(pdfPageToHtml(SAMPLE_PDF, info.pageCount)).rejects.toThrow('out of range');
  });

  it('throws on missing file', async () => {
    await expect(pdfPageToHtml('/tmp/does-not-exist.pdf', 0)).rejects.toThrow('Failed to open PDF');
  });
});
...
describe('pdfToHtmlStream', () => {
  it('yields all pages in order', async () => {
    const info = await pdfInfo(SMALL_PDF);
    const pages: number[] = [];

    for await (const page of pdfToHtmlStream(SMALL_PDF)) {
  ...
  it('each page carries correct pageCount', async () => {
    const info = await pdfInfo(SMALL_PDF);
    for await (const page of pdfToHtmlStream(SMALL_PDF, { endPage: 0 })) {
  ...
  it('respects startPage / endPage range', async () => {
    const pages: number[] = [];
    for await (const page of pdfToHtmlStream(SMALL_PDF, { startPage: 1, endPage: 2 })) {
      pages.push(page.pageIndex);
    }
  ...
  it('concurrency option prefetches multiple pages', async () => {
    ...
    for await (const page of pdfToHtmlStream(SMALL_PDF, { concurrency: 2 })) {
      pages.push(page.pageIndex);
    }

    // All pages yielded, in order
    expect(pages).toHaveLength(info.pageCount);
    expect(pages).toEqual([...Array(info.pageCount).keys()]);
  });
  ...
  it('throws on invalid range', async () => {
    async function drain() {
      for await (const _ of pdfToHtmlStream(SMALL_PDF, { startPage: 5, endPage: 2 })) {
        /* noop */
      }
    }
  ...
  it('propagates errors from missing file', async () => {
    async function drain() {
      for await (const _ of pdfToHtmlStream('/tmp/does-not-exist.pdf')) {
        /* noop */
      }
    }
...
describe('PdfWorkerPool', () => {
  it('converts PDFs with concurrency=1', async () => {
    ...
    const result = await pool.convert(SMALL_PDF, { startPage: 0, endPage: 0 });
    expect(result.pagesConverted).toBe(1);
    pool.destroy();
  });
  ...
  it('processes multiple PDFs respecting concurrency limit', async () => {
    ...
    const paths = [SMALL_PDF, SMALL_PDF, SMALL_PDF];

    const results = await Promise.all(paths.map((p) => pool.convert(p, { startPage: 0, endPage: 0 })));
  ...
  it('pool.active and pool.pending reflect in-flight state', async () => {
    ...
    // At some point, one should be active and one pending
  ...
  it('streams pages via pool', async () => {
    ...
    for await (const page of pool.stream(SMALL_PDF)) {
      pages.push(page.pageIndex);
    }
  ...
  it('destroy prevents further operations', async () => {
    const pool = new PdfWorkerPool({ concurrency: 1 });
    pool.destroy();
    await expect(pool.convert(SMALL_PDF)).rejects.toThrow('destroyed');
  });
...
describe('performance', () => {
  ...
  it('pdfToHtmlStream (concurrency=1): sequential yield time', async () => {
    ...
    const t0 = Date.now();
    let count = 0;
    for await (const _ of pdfToHtmlStream(SMALL_PDF)) count++;
    const elapsed = Date.now() - t0;
  ...
  it('pdfToHtmlStream (concurrency=4): prefetch reduces wall-clock time', async () => {
    const [t0] = [Date.now()];
    let c1 = 0;
    for await (const _ of pdfToHtmlStream(SMALL_PDF, { concurrency: 1 })) c1++;
    const seqMs = Date.now() - t0;

    const t1 = Date.now();
    let c4 = 0;
    for await (const _ of pdfToHtmlStream(SMALL_PDF, { concurrency: 4 })) c4++;
    const parMs = Date.now() - t1;

    console.info(`  stream c=1: ${seqMs}ms  c=4: ${parMs}ms  speedup: ${(seqMs / parMs).toFixed(2)}x`);
    expect(c1).toBe(c4); // same pages, just faster
  });

  it('writes output html to disk for manual inspection', async () => {
    const result = await pdfToHtml(SAMPLE_PDF, { startPage: 0, endPage: 10, dpi: 96 });
